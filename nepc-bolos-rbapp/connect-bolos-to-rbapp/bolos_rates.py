...
# %%
# This version uses reduced electric field values
enefile = []
ratefile = []
swarmfile = []
if __name__ == '__main__':
    EoN = np.linspace(0.01,2361, 500)
    press = 101325
    T_k = 300
    ND = press / co.k / T_k
...
# %%
# This version uses maxwellian
enefile = []
ratefile = []
swarmfile = []
if __name__ == '__main__':
    EoN = np.linspace(0.01,2361, 500)
    energy = np.linspace(0.01, 100, 500)
    press = 101325
    T_k = 300
    ND = press / co.k / T_k

    for ev in energy:
        g = grid.LinearGrid(0.01, 100, 500) # 0.01-100ev 500 intervals
        bsolver = solver.BoltzmannSolver(g)
        with open('Cross section.txt') as fp:
            processes = parser.parse(fp)
            bsolver.load_collisions(processes)
        bsolver.target['N2'].density = 1
        bsolver.kT = T_k * co.k / co.eV
        bsolver.EN = 300. * solver.TOWNSEND       
        bsolver.init()

        f0 = bsolver.maxwell(ev)
        f_sol = bsolver.converge(f0, maxn=200, rtol=1e-4)
        mean_energy = ev #bsolver.mean_energy(f_sol)
        # electron_temp = bsolver.electron_temperature(f_sol)     
        # newgrid = grid.QuadraticGrid(0, 20 * mean_energy, 200)
        # bsolver.grid = newgrid
        # bsolver.init()
        ...
        #enefile.append([td, mean_energy])
        ratefile.append([mean_energy,] + [bsolver.rate(f_sol, p) if bsolver.rate(f_sol, p) > 1.0e-25 else 0.0e00 for t, p in bsolver.iter_all()])
        #swarmfile.append([td, mean_energy, electron_temp, mu, diff, k])
...
# %%
# Puts the files in the matrix format
...
# %%
# Gives the files the right headers
